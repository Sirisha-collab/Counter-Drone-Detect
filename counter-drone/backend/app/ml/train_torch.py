...
def main() -> None:
    ...
    print(
        "\nRead this number sceptically. Training and testing both come from "
        "the same simulator, so it measures how well the model learned this "
        "simulator — not how it would do on a real sensor. The split-by-track "
        "above removes the worst of the leakage, not the rest."
    )
